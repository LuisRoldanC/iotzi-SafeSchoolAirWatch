...
client_id = "TEMP_Sensor_01" # ID Único para este sensor
mqtt_topic = b"iotzi/escuela/sensor/temp" # Tópico de Temperatura
...
# --- PROGRAMA PRINCIPAL (SOLO TEMPERATURA) ---
...
print("\n========================================")
print("🌡️ Sensor DHT - SOLO TEMPERATURA")
...
while True:
    try:
        ...
        if time.ticks_diff(current_time_ms, last_publish) >= publish_interval_ms:
            ...
            print(f"✓ LECTURA #{reading_count} | Temp: {temp}°C")

            # 2. CREACIÓN DEL JSON (TEMPERATURA)
            # Formato {"val": ...} para tu app.js
            payload_temp = {"val": temp}
            json_temp = json.dumps(payload_temp)
            
            # 3. PUBLICAR
            print(f"  📊 JSON Temp: {json_temp}")
            client.publish(mqtt_topic, json_temp)
